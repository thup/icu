...
import com.thup.icu.demo.Constants;
import lombok.Data;
...
public class CmdUtil {
    ...
    public static Process exec(String command) {
        ...
        try {
            ...
            System.out.println("windows命令执行错误: "+ e.getMessage());
        }
        return null;
    }
    ...
    /*public static Process openFile(File file) {
        if (file==null || !file.exists()) {
            System.out.println("命令执行错误: 文件不存在");
            return null;
        }
        String[] commands = null;
        String drive = getDrive(file.getPath());
        if (file.isDirectory()) {
    ...
    public static void printProcess(Process process) {
        ...
        try {
            if(FileUtil.isWin()){
                ...
                bufferedReader = new BufferedReader(new InputStreamReader(mergeInputStream));
            }
            String tmp = null;
    ...
    public static String formatValue(String value) {
        ...
        for (String s : Constants.SPECIAL_CHAR) {
            if (value.contains(s)) {
