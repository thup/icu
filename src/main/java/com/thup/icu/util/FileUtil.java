...
public class FileUtil {
		...
		if (file.exists()) {
			...
			if (!flag) {
				...
				if (files == null || files.length == 0) {
					flag = file.getParentFile().delete();
				}
			}
    ...
    public static String getStr(String desc) {
        return getStr(desc, "GB2312");
    }

    public static String getStr(String desc, String charSetName) {
	    if(Objects.isNull(charSetName)){
	        return desc;
        }
        if(isWin()){
            try {
                desc = new String(desc.getBytes(), charSetName);
